...
#
# Fail-path-first (A) before the real-tree assertion (C): a check that cannot catch the stale text
# in a fixture built to carry it would pass C for the wrong reason — a check that never runs.
...
check_methodology() {
  ...
  # 3. The "### debug-issue" narrative section said the same unconditional thing in prose.
  line=$(grep -n 'Terminal: it returns to whatever called it, carrying the cause' "$f") && {
