...
def skills_visible(project_root, skills):
    ...
    try:
        for skill in skills:
            ...
            dest = dest_dir / skill
            # A leftover from a crashed run is skipped, as it always was — but the consequence
            # changed with the mechanism: a leftover SYMLINK still resolved to the live source,
            # while a leftover COPY is frozen at the moment it was made. So a crash between
            # staging and teardown can now leave the bench measuring a stale SKILL.md — the same
            # class of silent mis-measurement this staging exists to prevent, one directory
            # closer. Deleting it unasked is the worse trade (it is indistinguishable from a
            # contributor's own .claude/skills entry, which must survive untouched), so it is
            # named here rather than guessed at; see the PR's follow-ups.
            if not source.is_dir() or dest.exists() or dest.is_symlink():
                continue
            # COPIED, not linked (#624). A symlink needs SeCreateSymbolicLink on Windows —
            # Developer Mode or elevation — which most machines do not grant, so `symlink_to`
            # raised `OSError: [WinError 1314]` and the run died before one trigger contract was
            # measured. #619 removed the same assumption from the two plugin trees by copying
            # (ADR 0017); this was the runtime caller left. A skill is a small tree of text, so
            # the copy is cheap, and the bench reads it read-only for the duration of one run.
            #
            # Appended to `made` BEFORE the copy, not after: `copytree` can raise partway through
            # a large or slow skill tree (disk full, EACCES, a Windows MAX_PATH), and a `made`
            # entry recorded only on success would drop that half-written `dest` from teardown —
            # exactly the stale-leftover hazard the comment above warns about, self-inflicted by
            # this same call. `rmtree(dest, ignore_errors=True)` is a safe no-op on a `dest` that
            # never got created at all, so appending early costs nothing on the success path.
            made.append(dest)
        ...
        for dest in made:
            # `made` holds ONLY what this call attempted to create, which is what keeps the
            # promise above: a real directory, or a link somebody else put there, was skipped by
            # the `dest.exists() or dest.is_symlink()` guard and never reaches this loop. So
            # everything here is a directory this call copied (fully or partially), and rmtree is
            # the whole teardown.
